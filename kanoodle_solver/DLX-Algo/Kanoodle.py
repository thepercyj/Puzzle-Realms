...
class Kanoodle:
    ...
    @staticmethod
    def createSearchRows(pieces, gridWidth, gridHeight):
        rotations = Kanoodle.Rotation
    ...
    def extractTiles(src, max):
        ...
        for c in src:
            if c == '\n':
                ...
                max.col = max(max.col, col + 1)
                max.row = max(max.row, row + 1)
    ...
    class Piece:
        ...
        def getWidth(self, r):
            if r == Kanoodle.Rotation.ROTATION_0 or r == Kanoodle.Rotation.ROTATION_180:
                return self.dimensions.col
            return self.dimensions.row
        ...
        def isTileAt(self, col, row, rotation, flipped):
            ...
            if rotation == Kanoodle.Rotation.ROTATION_0:
                ...
                if flipped:
                    localRow = self.getHeight() - 1 - localRow
            if localCol >= 0 and localRow >= 0 and localCol < self.getWidth() and localRow < self.getHeight():
